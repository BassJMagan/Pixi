...
class TaggerPredictor:
    ...
    def predict_tags(self, image):
        ...
        predictions = self.model.run([output_name], {input_name: self.prepare_image(image)})[0][0]
        return ", ".join([
            tag.replace("_", " ") for tag, score in zip(self.tag_names, predictions) if score > 0.30
        ])
